...
def normalize_loudness(loudness, min_value=5, max_value=65):
    """
    ...
    return min(int(scaled * 255), 255)


class VocalsExtractor(ExtractorBase):
    def __init__(self, data_senders):
        ExtractorBase.__init__(self, data_senders)
        self.rgb_receiver = NumpyArrayReceiver(data_senders.get("vocals_rgb_image"))
        self.bass_loudness_receiver = NumpyArrayReceiver(data_senders.get("stem_loudness_vocals"))
