...
class BassPerlinGenerator(GeneratorBase, CommandLineArgumentAdder):
    ...
    def __init__(self, data_senders):
        ...
        self.width = 10
        self.depth = 10
        self.rgb_image = NumpyArraySender((self.width, self.height, self.depth, 3))
    ...
    async def run_procedure(self):
        loudness = self.vocal_loudness_receiver.read_on_update()[0] / 5
        ...
        self.z_offset = self.y_offset + self.movement_speed * 3 + (self.movement_speed * loudness_scale * 7)

        red_noise = np.zeros((self.height, self.width, self.depth))
        green_noise = np.zeros((self.height, self.width, self.depth))
        blue_noise = np.zeros((self.height, self.width, self.depth))

        for y in range(self.height):
            for x in range(self.width):
                for z in range(self.depth):
                    red_noise[y][x][z] = noise.snoise3(
                        (x + self.x_offset) / scale * 0.1,
                        (y + self.y_offset) / scale * 0.2,
                        (z + self.z_offset) / scale * 0.3,
                        octaves=1,
                    )
                    green_noise[y][x][z] = noise.snoise3(
                        (x + self.x_offset) / scale * 0.3,
                        (y + self.y_offset) / scale * 0.4,
                        (z + self.z_offset) / scale * 0.5,
                        octaves=2
                    )
                    blue_noise[y][x][z] = noise.snoise3(
                        (x + self.x_offset) / scale * 0.5,
                        (y + self.y_offset) / scale * 0.6,
                        (z + self.z_offset) / scale * 0.7,
                        octaves=3
                    )
